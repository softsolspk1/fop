...
async function main() {
    ...
    // 2. Read JSON
    const extractedFacultyPath = path.join(__dirname, '..', '..', 'scripts', 'extracted_faculty.json');
    const extractedCoursesPath = path.join(__dirname, '..', '..', 'scripts', 'extracted_courses.json');
    const facultyData = JSON.parse(fs.readFileSync(extractedFacultyPath, 'utf-8'));
    const coursesData = JSON.parse(fs.readFileSync(extractedCoursesPath, 'utf-8'));
    ...
    for (const course of coursesData) {
        ...
        const creditHoursStr = String(course.creditHours).replace(/[^\d.]/g, '');
        const creditHours = parseFloat(creditHoursStr) || 0;
