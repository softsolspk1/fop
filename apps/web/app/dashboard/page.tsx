    ...
    const fetchData = async () => {
      try {
        if (user?.role === 'SUPER_ADMIN' || user?.role === 'DEPT_ADMIN') {
          ...
          // In a real app, we'd fetch specific student stats here
          setDashboardStats({
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-6">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-4 p-5 rounded-3xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 cursor-default group">
                  ...
                  <div className="flex-1">
                    <h4 className="font-black text-slate-800">Advanced Pharmacology - Session {i}</h4>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-tighter flex items-center gap-2">
                       <Clock className="w-3 h-3 text-blue-600" /> Live Session • Dr. Sarah Ahmed
                    </p>
                  </div>
                  <button className="px-6 py-2.5 bg-slate-800 text-white text-[10px] font-black rounded-xl shadow-lg shadow-slate-200 uppercase tracking-widest hover:bg-slate-900 transition-all">Monitor</button>
                </div>
              ))}
            </div>
          </div>
