  ...
  return (
    <div className="w-72 bg-white h-screen border-r border-slate-100 flex flex-col fixed left-0 top-0 z-40">
      <div className="p-6 flex flex-col flex-1 overflow-hidden">
        <div className="flex items-center gap-3 mb-8 px-2">
          ...
          <div className="overflow-hidden">
            <h1 className="font-black text-slate-800 tracking-tight leading-none text-lg uppercase truncate">FOP UOK</h1>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 truncate">Faculty of Pharmacy</p>
          </div>
        </div>

        <nav className="flex-1 overflow-y-auto pr-2 -mr-2 space-y-1 custom-scrollbar">
          {filteredNavItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[13px] font-bold tracking-tight ${isActive ? 'opacity-100' : 'opacity-90'}`}>{item.label}</span>
                {isActive && (
                  <motion.div 
                    layoutId="activeTab"
      ...
      <div className="p-6 border-t border-slate-50 bg-white">
        <button 
          onClick={logout}
          className="flex items-center gap-3 px-4 py-3 w-full text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-300 group font-bold text-sm"
        >
